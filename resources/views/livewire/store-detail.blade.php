                ...
                <div @click="$dispatch('open-product-modal', { productId: {{ $product->id }} })" 
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if($product->quantity > 0)
                                <button @click.stop="$store.cart.toggle({id: {{ $product->id }}, name: `{{ $product->name }}`, price: {{ $product->price }}, image: `{{ $product->image ? Storage::url($product->image) : '' }}`})" 
                                    class="p-2 rounded-full transition-colors relative"
                                    :class="$store.cart.has({{ $product->id }}) ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-pastel-cream hover:bg-pastel-yellow text-slate-700'">
                                    ...
                                    <svg x-show="$store.cart.has({{ $product->id }})" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5" style="display: none;">
